...
use notify_debouncer_full::{notify::*, new_debouncer, DebounceEventResult, Debouncer, new_debouncer_opt};
...
pub fn create_file_name(file_path: &String, backup_save_path: String) -> String {
    ...
    let backup_save_path = backup_save_path + "/" + new_file_name.as_str();
    println!("New Save Path: {}", backup_save_path);
...
pub fn watch_poller<P: AsRef<Path>>(path: P) -> notify::Result<()> {
    let (tx, rx) = channel();
    // use the PollWatcher and disable automatic polling
